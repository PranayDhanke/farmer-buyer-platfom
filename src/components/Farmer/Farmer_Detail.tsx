...
const Farmer_Detail = ({ farmer_name }: { farmer_name: string }) => {
  ...
  const filteredProducts = products.filter((product) => {
    ...
    const isFarmers = 
      product.farmer.toLowerCase().includes(decoded_name.toLowerCase());
          ...
          <div className="bg-white rounded-xl overflow-hidden shadow-md p-6 mb-8">
            <div className="flex items-center mb-6">
              ...
              <div>
                ...
                <div className="flex items-center text-yellow-500 mb-2">
                  {[...Array(Math.floor(selectedFarmer.rating))].map((_, index) => (
                    <FaStar key={index} />
                  ))}
                  {selectedFarmer.rating % 1 > 0 && <FaStar className="text-gray-400" />}
                  <span className="ml-2 text-gray-600">Rating: {selectedFarmer.rating}</span>
                </div>
              </div>
            </div>
            ...
            <div className="mb-4">
              ...
              <p className="text-gray-600 text-sm">
                {selectedFarmer.address.taluka}, {selectedFarmer.address.district}, {selectedFarmer.address.city}, {selectedFarmer.address.state}
              </p>
            </div>

            <div className="mb-4">
              <h3 className="font-bold text-lg text-gray-700">Farming Details</h3>
              <p className="text-gray-600 text-sm">Main Crop: {selectedFarmer.mainCrop}</p>
              <p className="text-gray-600 text-sm">Farm Type : {selectedFarmer.farmType}</p>
              <p className="text-gray-600 text-sm">Farmer Rating: {selectedFarmer.rating}</p>
            </div>
