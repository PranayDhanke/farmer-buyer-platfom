...
async function uploadAndGetUrl(file: File, path: string): Promise<string | null> {
  const { error } = await supabase.storage.from("farmer").upload(path, file, { upsert: true });
  if (error) {
    console.error(`Upload error at ${path}:`, error);
    return error.name;
  }

  const { data } = supabase.storage.from("farmer").getPublicUrl(path);
...
export async function POST(req: NextRequest) {
  try {
    ...
    const mainCrops = formData.get("mainCrops")?.toString() ?? "";
    const farmType = formData.get("farmType")?.toString() ?? "";
    ...
    if (profilePhoto && profilePhoto.size > 0) {
      const profileUrl = await uploadAndGetUrl(profilePhoto, `${uid}/profile.jpg`);
